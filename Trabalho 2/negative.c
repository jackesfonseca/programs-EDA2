...
int main(void)
{
	...
	Pilha *pilha;
	FilaPrio *fp;
	int TABLE_SIZE = 1000, EDAzinhos = 1, cont_turnos = 0, pos_row, pos_column;
	char command[256];
	...
	/* valores iniciais */
	scanf("%d %d %d %d", &pos_row, &pos_column, &p_i, &t_loop); /* lê dados iniciais*/
	if(pos_row<0)
		row = row * (-1);
	if(pos_column<0)
		column = column * (-1);
	matriz[row][column].row = pos_row;
	matriz[row][column].column = pos_column;
